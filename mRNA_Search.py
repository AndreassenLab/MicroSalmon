...
for line in lines[1:]:
    ...
    accs_to_seqid[line.split("\t")[0]]=line.split("\t")[1]
...
for counter, query in enumerate(queries):
    ...
    skip = 0
    if not args.seqid and query in accs_to_seqid:
        seqid = accs_to_seqid[query]
        print(query)
    elif args.seqid and query in annotation_dict:
        seqid = query
        print(query)
    else:
        print(query + " not found. Check spelling and if search mode is set to Accession Number or SeqID")
        skip = 1

    if skip == 0:
        output = "Accession Number: " + annotation_dict[seqid][0] + "\nSeqID in TSA: " + seqid + "\n3'UTR length: " + str(len(utr_dict[seqid])) +  "\n"
        if not args.utr:
        ...
        if not args.mirna:
            if seqid in RNAhybrid_dict:
                output += "\nNumber of predicted MREs: " + str(len(RNAhybrid_dict[seqid])) + "\nmiRNAs predicted to bind MREs: "
                mirnas = set()
