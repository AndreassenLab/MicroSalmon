...
parser.add_argument("-q", "--query", help="Query IDs, one GO ID, or a list of multiple GO IDs in quotes separated by spaces, e.g -q \"GO:0000050 GO:0004053\"")
parser.add_argument("-i", "--query_file", help="Query filename. Must be a list of GO IDs, one per line. At least one of Input_File or Query is required.")
...
for line in lines[1:]:
    ...
    for counter, GOID in enumerate(line.split("\t")[6].split(",")):
        if GOID not in GO_dict:
            GO_dict[GOID] = [line.split("\t")[1]]
            GOID_to_GOT[GOID] = line.split("\t")[7].split(",")[counter]
        else:
            GO_dict[GOID] += [line.split("\t")[1]]
...
for counter, query in enumerate(queries):
    ...
    if query in GO_dict:
        ...
        skip = 1
    if skip == 0:
        output = "Input GO ID: " + query + "\nGO Term: " + GOID_to_GOT[query] + "\n"
        if not args.target_summary:
            accs = []
            genesordered = []
            genes = set()
            MREs = set()
            for seqid in sorted(GO_dict[query]):
            ...
            output += "\nNumber of Annotated Transcripts: " + str(len(sorted(GO_dict[query]))) + "\nNumber of unique target genes: " +str(len(genes)) + "\nTranscript Accessions: " + ";".join(accs) + "\nTranscript SeqIDs in TSA: " + ";".join(sorted(GO_dict[query])) + "\nTranscript Annotated Genes: " + ";".join(genesordered)
            output +=  "\n\nNumber of predicted MREs for GO: " + str(len(MREs)) + "\nmiRNAs predicted to bind MREs: " + ";".join(MREs) + "\n"
        for counter, seqid in enumerate(sorted(GO_dict[query])):
